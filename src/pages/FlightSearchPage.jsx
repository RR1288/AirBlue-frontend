...
const FlightSearchPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Query the flight offers using the request_id.
      const limit = 10;
  ...
  const handleHold = async (flightId, passengerId) => {
    try {
      ...
      const passenger = {
        email: username,
        // Replace these with actual values or data from local storage as needed
        family_name: "Stark",
        given_name: "Tony",
        born_on: "1980-07-24",
        title: "mr",
      ...
      const holdEndpoint = `/flights/${encodeURIComponent(flightId)}/hold`;
      ...
      addNotification({
        title: "Flight Held",
        message: "Your flight has been held successfully.",
        type: "success",
      });

      // Redirect the user to the "my-events" page on success
      navigate("/my-events");
    } catch (err) {
              ...
              <div key={flight.id} className={styles.card}>
                ...
                {flight.slices.length > 1 && (
                  <div className={styles.flightSegment}>
                    ...
                    <p className={styles.cardText}>
                      {flight.slices[1].origin.iata_code}{" "}
                      <FontAwesomeIcon icon={faArrowRight} className={styles.icon} />{" "}
                      {flight.slices[1].destination.iata_code}
                    </p>
                    <p className={styles.cardText}>
                      Duration: {flight.slices[1].duration}
                    </p>
                  </div>
                )}
                <button
                  onClick={() =>
                    handleHold(flight.id, flight?.passengers[0]?.id)
                  }
                  disabled={holdLoading}
                  className={styles.holdButton}
                >
                  {holdLoading ? "Please wait..." : "Hold"}
                </button>
              </div>
            ))
          ) : (
