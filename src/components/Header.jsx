...
function Header({ title, userRole, hideSidebar = false, onRoleChange }) {
  ...
  const availableRoles = useMemo(() => {
    ...
    return stored.split("").map(letter => roleMap[letter]).filter(Boolean);
  }, []);
  ...
  return (
    <>
      ...
      <header className={styles.header}>
        {!hideSidebar && (
          <button onClick={() => setSidebarOpen(!sidebarOpen)} className={styles.menuButton}>
        ...
        {availableRoles.length > 1 && (
          <select value={userRole} onChange={handleRoleChange} className={styles.roleSwitcher}>
